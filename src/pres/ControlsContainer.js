...
export default class ControlsContainer extends React.Component {
      ...
      <div>
        <Modal isOpen={this.state.activeGame} toggle={this.toggleModal}>
          ...
          {!this.state.activeGame ? null : (
            <Table>
              <TableBody>
                {Object.entries(this.state.activeGame.headers).map((entry) =>
                  !entry[1] ? null : (
                    <TableRow className="performanceRatingRow">
                      <TableCell className="performanceRatingRow">
                        {entry[0]}
                      </TableCell>
                      <TableCell className="performanceRatingRow">
                        {entry[1]}
        ...
        <Nav tabs>
          <NavItem>
            ...
            >
              ...
              {this.state.activeTab === "user" ? "User" : ""}
            </NavLink>
          </NavItem>
          <NavItem>
            <NavLink
              className={classnames({
                active: this.state.activeTab === "moves",
              })}
              onClick={() => {
                this.toggle("moves");
              }}
            >
              ...
              {this.state.activeTab === "moves" ? "Moves" : ""}
            </NavLink>
          </NavItem>
          <NavItem>
            <NavLink
              className={classnames({
                active: this.state.activeTab === "book",
              })}
              onClick={() => {
                this.toggle("book");
              }}
            >
              ...
              {this.state.activeTab === "book" ? "Opening book" : ""}
            </NavLink>
          </NavItem>
          <NavItem>
            <NavLink
              className={classnames({
                active: this.state.activeTab === "report",
              })}
              onClick={() => {
                this.toggle("report");
              }}
            >
              ...
              {this.state.activeTab === "report" ? "Report" : ""}
            </NavLink>
          </NavItem>
          <NavItem>
            <NavLink
            ...
            >
              ...
              {this.state.activeTab === "settings" ? "Controls" : ""}
            </NavLink>
          </NavItem>
        </Nav>
        <TabContent activeTab={this.state.activeTab}>
          ...
          <TabPane tabId="report">
            <ReportControls
              ...
              moveDetails={this.props.openingGraph.getDetailsForFen(
                this.props.fen
              )}
              launchGame={this.launchGame.bind(this)}
              settings={this.props.settings}
  ...
  reportFooter() {
    return (
      <span>
        Performance rating calculated based on{" "}
        <a
