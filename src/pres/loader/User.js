...
const DEFAULT_CHESSCOM_PIC_URL =
  "https://images-ext-2.discordapp.net/external/aStnmplCXfQgwmEGD3cZlkuUByRcX4P5YMTj1YTS6oo/https/i.imgur.com/tnc2dmO.png?format=webp&quality=lossless&width=800&height=800";
...
export default class User extends React.Component {
  ...
  validateInputDetailsSet() {
    if (this.props.site === Constants.SITE_EVENT_DB) {
      ...
      this.props.site === Constants.SITE_LICHESS ||
      ...
      if (!this.state.tournamentUrl) {
        ...
        if (hostname === "lichess.org" || hostname.endsWith(".lichess.org")) {
          ...
          this.setState({
            tournamentUrlError:
              "chess.com tournaments are not currently supported",
          });
          return false;
        }
        if (!tournamentSite) {
    ...
    if (this.props.site === Constants.SITE_PLAYER_DB) {
      ...
      if (this.props.playerName) {
        if (this.props.playerExists === true) {
          const profilePicUrl =
            this.props.profilePicUrl ?? DEFAULT_CHESSCOM_PIC_URL;
          const profilePic = (
            <img
              alt={this.props.playerName}
              className="siteimage"
          ...
          return (
            <span>
              {getNumberIcon("done")}
              {profilePic} {"  "} <b>{this.props.playerName}</b>
            </span>
          );
        } else if (this.props.playerExists === false) {
          return (
            <Tooltip title="Couldn't find this user!">
              <span>
                {getNumberIcon("done")} ❌ <b>{this.props.playerName}</b>
              </span>
            </Tooltip>
          );
        }
      ...
      this.props.site === Constants.SITE_PGN_FILE ||
      this.props.site === Constants.SITE_OPENING_TREE_FILE
    ) {
      if (this.props.files.length === 1) {
        return (
          <span>
            {getNumberIcon("done")}File: <b>{this.props.files[0].name}</b>
          </span>
        );
      }
      if (this.props.files.length > 1) {
        return (
          <span>
            {getNumberIcon("done")}
            {this.props.files.length} PGN files uploaded
          </span>
        );
      }
      ...
      if (this.props.selectedOnlineTournament) {
        return (
          <span>
            {getNumberIcon("done")}Id:{" "}
            <b>{this.props.selectedOnlineTournament.tournamentId}</b>
          </span>
        );
      }
    }
  ...
  title(site) {
    if (
      site === Constants.SITE_ONLINE_TOURNAMENTS ||
      site === Constants.SITE_EVENT_DB
    ) {
      return "Tournament details";
    } else if (
      site === Constants.SITE_OPENING_TREE_FILE ||
  ...
  launchLichessOauth() {
    console.log(this);
    setTimeout(
      this.props.oauthManager.fetchAuthorizationCode.bind(
  ...
  getLichessCardBody() {
    if (this.props.lichessLoginState === Constants.LICHESS_STATE_PENDING) {
      return (
        <Card>
          <div className="center">
            <Spinner className="bigSpinner dividerMargin" />
            <br />
          </div>
        </Card>
      );
    } else if (
      this.props.lichessLoginState === Constants.LICHESS_FAILED_FETCH
    ) {
      return (
        <Card>
          <CardBody className="singlePadding">
            <CardTitle className="smallBottomMargin redColor">
              <FontAwesomeIcon icon={faInfoCircle} className="lowOpacity" />{" "}
              Failed to fetch login status
            </CardTitle>
            <CardTitle className="smallBottomMargin">
              <FontAwesomeIcon
                icon={faSync}
                className="lowOpacity smallText leftMargin2"
              />
              <span
                onClick={this.props.refreshLichessStatus}
                className="smallText linkStyle leftMargin4"
              >
                {" "}
      ...
      this.props.lichessLoginState === Constants.LICHESS_LOGGED_IN &&
      this.props.lichessLoginName
    ) {
      return (
        <Card>
          <CardBody className="singlePadding">
            <CardTitle>
              <FontAwesomeIcon
                icon={faCheck}
                className="lowOpacity greenColor"
              />{" "}
              Logged in as
    ...
    return (
      <Card>
        <CardBody className="singlePadding">
          <CardTitle className="smallBottomMargin">
            <FontAwesomeIcon icon={faInfoCircle} className="lowOpacity" /> Speed
            up tree building (optional)
          </CardTitle>
          <CardText className="smallText">
            Lichess allows much faster download of games if you login. You can
            ...
            . Recommended for viewing your own games or when your opponent has
            lots of games.
          </CardText>
          <MaterialUIButton
            onClick={this.launchLichessOauth.bind(this)}
  ...
  getPlayerNameInput(label, helperText) {
    return (
      <TextField
        ...
        margin="dense"
        onChange={this.editPlayerName.bind(this)}
        label={label}
        variant="outlined"
        value={this.state.playerName}
        helperText={
          this.state.playerNameError ? this.state.playerNameError : helperText
        }
        error={this.state.playerNameError ? true : false}
        onKeyUp={this.playerNameKeyUp.bind(this)}
      />
    );
  }
  ...
  getOnlineTournamentSelection() {
    return (
      <div>
        ...
        <br />
        {this.getOnlineTournamentInput(
          "Enter tournament url",
          "eg. https://lichess.org/tournament/QlooVt7W"
        )}
      </div>
    );
  }
  getOnlineTournamentCard() {
    return (
      <Card>
        <CardBody className="singlePadding">
          <CardTitle className="smallBottomMargin">
            <FontAwesomeIcon icon={faInfoCircle} className="lowOpacity" /> How
            it works
          </CardTitle>
          <CardText className="smallText">
            You can load all of the games of a lichess tournament by copying the
            url from your address bar on those sites and pasting it below.
            <br />
            <br />
            <b>Why is chess.com not supported?</b>
            <br />
            chess.com API has a few bugs in returning tournament games so we are
            not able to support them currently. Bugs have been reported to
            chess.com.
          </CardText>
        </CardBody>
      </Card>
  ...
  getOnlineTournamentInput(label, helperText) {
    return (
      <TextField
        ...
        id="onlineTournamentTextBox"
        margin="dense"
        onChange={this.editTournamentUrl.bind(this)}
        label={label}
        variant="outlined"
        value={this.state.tournamentUrl}
        helperText={
          this.state.tournamentUrlError
            ? this.state.tournamentUrlError
            : helperText
        }
        error={this.state.tournamentUrlError ? true : false}
        onKeyUp={this.playerNameKeyUp.bind(this)}
      />
    );
  }
      ...
      <div>
        <Card>
          <CardBody className="singlePadding">
            ...
            <CardText className="smallText">
              If you plan to revisit the same player, you can save a{" "}
              <b>.tree</b> file locally by loading a tree and then clicking{" "}
              <i>"Save .tree file"</i>. To reload the same tree, drop the{" "}
              <b>.tree</b> file in the dropzone below.
            </CardText>
          </CardBody>
        </Card>
        <br />
  ...
  render() {
    return (
      <Accordion
        expanded={this.props.expandedPanel === "user"}
      ...
      >
        ...
        <AccordionDetails>{this.getInputsToShow()}</AccordionDetails>
        <Divider />
        <AccordionActions>
          <MaterialUIButton
            size="small"
            color="primary"
            onClick={this.setPlayerDetails.bind(this)}
          >
            Continue
          </MaterialUIButton>
        </AccordionActions>
