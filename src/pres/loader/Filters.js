...
export default class Filters extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      playerColor: this.props.playerColor ?? "white",
    ...
    if (this.props.playerColor && !isDisabled) {
      return (
        <span>
          ...
          Color:{" "}
          <b>
            {this.props.playerColor === Constants.PLAYER_COLOR_WHITE
              ? "White"
              : "Black"}{" "}
          </b>
          {this.areAdvancedFiltersApplied() ? (
            <span className="smallText">(Filters applied)</span>
          ) : null}
        </span>
      );
    }
  ...
  componentWillReceiveProps(newProps) {
    this.setState({
      ...newProps.advancedFilters,
      playerColor: newProps.playerColor,
    });
  }
  ...
  render() {
    let isDisabled = !SitePolicy.isFilterPanelEnabled(
      this.props.site,
    ...
    return (
      <Accordion
        expanded={this.props.expandedPanel === "filters"}
        ...
        <AccordionDetails>
          <div className="pgnloadersection">
            <FormControl component="fieldset" error={!!this.state.colorError}>
              ...
              <RadioGroup
                onChange={this.playerColorChange.bind(this)}
                value={this.state.playerColor}
              >
                <FormControlLabel
                  className="whitelabel"
                  control={<Radio color="primary" />}
                  value={Constants.PLAYER_COLOR_WHITE}
                  label={
                    this.state.playerColor === Constants.PLAYER_COLOR_WHITE ? (
                      <b>White</b>
                    ) : (
                      "White"
                    )
                ...
                />
                <FormControlLabel
                  className="blacklabel"
                  control={<Radio color="primary" />}
                  value={Constants.PLAYER_COLOR_BLACK}
                  label={
                    this.state.playerColor === Constants.PLAYER_COLOR_BLACK ? (
                      <b>Black</b>
                    ) : (
                      "Black"
                    )
          ...
          {SitePolicy.isAdvancedFiltersEnabled(this.props.site) ? (
            <div className="pgnloadersection">
              <span
                className="linkStyle"
                onClick={this.toggleState("isAdvancedFiltersOpen").bind(this)}
              >
                Advanced filters{" "}
                <FontAwesomeIcon
                  icon={
                    this.state.isAdvancedFiltersOpen ? faCaretUp : faCaretDown
                  }
                />{" "}
              </span>
              <Collapse isOpen={this.state.isAdvancedFiltersOpen}>
                <AdvancedFilters
                  ...
                  handleTimeControlChange={this.handleTimeControlChange.bind(
                    this
                  )}
                  handleEloRangeChange={this.handleEloRangeChange.bind(this)}
                  handleOpponentNameChange={this.handleOpponentNameChange.bind(
                    this
                  )}
                  handleDownloadLimitChange={this.handleDownloadLimitChange.bind(
                    this
                  )}
