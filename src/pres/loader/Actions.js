...
import {
  serializeOpeningTree,
  deserializeOpeningTree,
} from "../../app/OpeningTreeSerializer";
...
export default class Actions extends React.Component {
  ...
  importOpeningTree() {
    deserializeOpeningTree(this.props.files[0], (err, data, subMesage) => {
      if (err) {
  ...
  exportOpeningTree() {
    serializeOpeningTree(
      this.props.exportOpeningTreeObject(),
  ...
  getPgnString(game) {
    return `${Object.entries(game.headers)
      .map((header) => `[${header[0]} "${header[1]}"]`)
      .join("\n")}
                \n${game.moves
                  .map((moveObject, index) => {
  ...
  downloadResponse(result, pendingDownloads) {
    this.fileWriter
      .write(
        this.encoder.encode(
          result.map((game) => this.getPgnString(game)).join("")
        )
      )
      .then(() => {
        if (!pendingDownloads) {
  ...
  readPgn(shouldDownloadToFile) {
    ...
    new PGNReader(this.props.variant).then((readerInstance) => {
      ...
      this.pgnReader.fetchPGNFromSite(
        this.props.playerName,
        this.props.playerColor,
        this.props.site,
    ...
    return (
      <div className="pgnloadersection">
        <MaterialUIButton
          ...
          color="primary"
          startIcon={
            this.state.exportingInProgress ? <HourglassEmptyIcon /> : <GetApp />
          }
          className="mainButton"
          disableElevation
          disabled={this.state.exportingInProgress}
        >
          {this.state.exportingInProgress
            ? "Loading from file"
            : "Load openingtree"}
        </MaterialUIButton>
      </div>
          ...
          <div className="pgnloadersection">
            <Tooltip
              ...
              title={
                ...
                "Save a .tree file locally for faster reload later"
              }
            >
              <span>
                <MaterialUIButton
                  ...
                  disableElevation
                  disabled={
                    !!downloadDisabledReason || this.state.exportingInProgress
                  }
                >
                  {this.state.exportingInProgress
                    ? "Saving to file"
          ...
          <div>
            <div className="pgnloadersection">
              {!this.props.playerColor
                ...
                ? "White games"
                : "Black games"}
              {` loaded: ${this.props.gamesProcessed} `}
              {this.props.isDownloading ? (
                <span className="stopDownloading">
                  [
                  <span
                    className="linkStyle"
                    onClick={this.stopDownloadingAction.bind(this)}
                  >
                    <img
                      alt="loading spinner"
                      src="./spinner.gif"
            ...
            <div
              onClick={() => this.props.switchToMovesTab()}
              className="navLinkButton pgnloadersection"
            >
  ...
  render() {
    if (this.props.expandedPanel) {
      return (
        <Fade timeout={Constants.LOADER_ANIMATION_DURATION_MS * 2}>
          {this.mainComponent()}
        </Fade>
      );
    }
    return (
      <Fade in timeout={Constants.LOADER_ANIMATION_DURATION_MS * 3}>
        {this.mainComponent()}
      </Fade>
    );
  }
