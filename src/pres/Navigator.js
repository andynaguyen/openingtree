      ...
      <Container id="navigator">
        ...
        {this.openingManager.pgnListSoFar().map((move, index) => (
          <Row key={`${move.moveNumber}`} className="navCol">
            ...
            <Col
              sm="5"
              className={`navItem navMove border ${
                this.openingManager.currentIndex - 1 === index * 2
                  ? "selectedMove"
                  : ""
              }`}
            ...
            <Col
              sm="5"
              className={`navItem navMove border ${
                this.openingManager.currentIndex - 1 === index * 2 + 1
                  ? "selectedMove"
                  : ""
              }`}
              onClick={this.moveTo(index * 2 + 1).bind(this)}
            >
              {`${move.blackPly}`}
            </Col>
          </Row>
