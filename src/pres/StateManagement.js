...
function updateProcessedGames(downloadLimit, n, parsedGame) {
  ...
  this.state.openingGraph.addPGN(
    parsedGame.pgnStats,
  ...
  this.setState({
    ...
    downloadingGames:
      totalGamesProcessed < downloadLimit ||
      downloadLimit >= Constants.MAX_DOWNLOAD_LIMIT
        ? this.state.downloadingGames
        : false,
  });
    ...
    shapes = shapes.concat(
      moves
        .filter((m) => {
          ...
          if (
            highlightedMove.orig === m.orig &&
            highlightedMove.dest === m.dest
          ) {
            return false;
          }
          return true;
...
function showInfo(message, trackingLabel) {
  this.setState({
    message: message,
    messageSeverity: Constants.ERROR_SEVERITY_SUCCESS,
  });
}
  ...
  if (window.innerWidth <= 768) {
    ...
    return Math.round(
      Math.min(
        ...
        Math.max(512, window.innerHeight - 100)
      )
    ); // 40vw
  } else {
...
function getRedditLink() {
  return `https://www.reddit.com/message/compose/?to=${
    Constants.OPENING_TREE_REDDIT
  }&subject=${this.getSubject()}&message=%0D%0A%0D%0A%0D%0A${this.getBody()}`;
}

function getEmailLink() {
  return `mailto:${
    Constants.OPENING_TREE_EMAIL
  }?subject=${this.getSubject()}&body=%0D%0A%0D%0A%0D%0A${this.getBody()}`;
}

function getSubject() {
  return this.state.diagnosticsDataOpen
    ? "Possible Openingtree bug"
    : "Feedback on Openingtree";
}
function getBody() {
...
function variantChange(newVariant) {
  this.setState({
    variant: newVariant,
    openingGraph: new OpeningGraph(newVariant),
  });
...
function getBookMoves() {
  ...
  if (
    ...
    Constants.OPENING_BOOK_TYPE_OFF
  ) {
    return { fetch: "off" };
  }
...
function getCompareScores(move) {
  return (
    ((move.details.whiteWins + move.details.draws / 2) / move.details.count) *
    100
  );
}
