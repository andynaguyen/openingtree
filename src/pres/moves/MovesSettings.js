...
export default class MovesSettings extends React.Component {
  constructor(props) {
    ...
    mSettings.openingBookType = this.getTransformedBookType(
      mSettings.openingBookType
    );
    this.state = {
  ...
  set() {
    ...
    this.props.settingsChange(
      Constants.SETTING_NAME_MOVES_SETTINGS,
      this.state.movesSettings
    );
    this.props.toggle();
  }
  ...
  render() {
    return (
      <Modal isOpen={this.props.isOpen} toggle={this.cancel.bind(this)}>
        <ModalHeader toggle={this.cancel.bind(this)}>
          Opening book settings
        </ModalHeader>
        <ModalBody>
          <div className="littlePaddingTop">{this.getOpeningBookType()}</div>
          {this.state.movesSettings.openingBookType ===
          Constants.OPENING_BOOK_TYPE_LICHESS ? (
            <div className="littlePaddingTop">
          ...
          <div className="littlePaddingTop">
            {this.getIndicatorSwitch(
              "openingBookWinsIndicator",
              <span>
                Book indicators{" "}
                <Link
                  href="#"
                  className="smallText"
                  onClick={this.toggleIndicatorInfo.bind(this)}
                >
                  whats this?
                </Link>
              </span>
            ...
            <Card>
              <CardBody className="singlePadding">
                ...
                <CardText className="smallText">
                  Little triangles indicate the openingbook statistics on the
                  moves tab and the player statistics on the openingbook tab.{" "}
                  <br />
                  Everything left of the white triangle is a win for white,
                  everything right of the black triangle is a win for black and
                  everything in the middle is a draw.
                  <br />
                  If there is only a black triangle and no white, it means there
                  are no draws.
                </CardText>
              </CardBody>
            </Card>
        ...
        <ModalFooter>
          <Button color="link" onClick={this.cancel.bind(this)}>
            Cancel
          </Button>
          <Button color="primary" onClick={this.set.bind(this)}>
            Set
          </Button>
        </ModalFooter>
      </Modal>
  ...
  getTransformedBookType(value) {
    ...
    if (
      value === Constants.OPENING_BOOK_TYPE_MASTERS &&
    ...
    return (
      <div>
        ...
        <ToggleButtonGroup
          size="small"
          exclusive={true}
          value={this.getTransformedBookType(
            this.state.movesSettings.openingBookType
          )}
          onChange={this.updateMoveSetting("openingBookType")}
          aria-label="book type"
        >
          <ToggleButton
            value={Constants.OPENING_BOOK_TYPE_OFF}
            aria-label={Constants.OPENING_BOOK_TYPE_OFF}
          >
            Off
          </ToggleButton>{" "}
          {this.mastersBookAvailable() ? (
            <ToggleButton
              value={Constants.OPENING_BOOK_TYPE_MASTERS}
              aria-label={Constants.OPENING_BOOK_TYPE_MASTERS}
            >
              Masters
            </ToggleButton>
          ) : null}
          <ToggleButton
            value={Constants.OPENING_BOOK_TYPE_LICHESS}
            aria-label={Constants.OPENING_BOOK_TYPE_LICHESS}
          >
            Lichess
          </ToggleButton>
        </ToggleButtonGroup>
  ...
  getOpeningBookRating() {
    return (
      <div>
        ...
        <ToggleButtonGroup
          size="small"
          value={this.state.movesSettings.openingBookRating}
          onChange={this.updateMoveSetting("openingBookRating")}
  ...
  getOpeningBookTimeControls() {
    return (
      <div>
        ...
        <ToggleButtonGroup
          size="small"
          value={this.state.movesSettings.openingBookTimeControls}
          onChange={this.updateMoveSetting("openingBookTimeControls")}
          aria-label="text formatting"
        >
