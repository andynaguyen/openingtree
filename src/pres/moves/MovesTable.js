...
export default class MovesTable extends React.Component {
  ...
  compareScores(currMove) {
    ...
    return (
      <ProgressBar percent={0} stepPositions={values}>
        {this.getIndicator(
          "./images/book.png",
          "12",
          "12",
  ...
  compareProgress(currMove) {
    ...
    return (
      <ProgressBar percent={0} stepPositions={compareTo.values}>
        {this.getIndicator(
  ...
  getPopover(moveIndex) {
    ...
    return (
      <Popover
        trigger="hover"
        placement="right"
        isOpen={performancePopoverOpen}
        target={`p${this.props.namespace}${moveIndex}`}
        toggle={this.togglePerformancePopover(moveIndex)}
      >
        <ReportControls
          moveDetails={openMove.details}
          simplifiedView={true}
          isOpen={performancePopoverOpen}
          launchGame={this.props.launchGame}
          settings={this.props.settings}
          reportFooter={this.reportFooter(moveIndex)}
        />
      </Popover>
    );
  }
  ...
  reportFooter(moveIndex) {
    ...
    if (this.getTranspositionWarningLevel(moveIndex) !== "none") {
      return (
        <div>
          {this.getInfoIcon(moveIndex)}
          <b> This move has transpositions</b>{" "}
          <div>
            {currMove.san} has been played{" "}
            {currMove.moveCount === 1 ? `once` : `${currMove.moveCount} times`}{" "}
            in this position but the resulting position has appeared{" "}
            {currMove.details.count} times through other move orders.
          </div>
        </div>
      );
    }
  ...
  render() {
    ...
    if (!hasMoves)
      return offCard(
        ...
        <b>
          ...
          <MovesSettings
            isOpen={this.state.moveSettingsOpen}
    ...
    return (
      <Table>
          ...
          <TableRow>
            ...
            <TableCell>
              ...
              <FontAwesomeIcon
                ...
                icon={faWrench}
                onClick={this.toggleMovesSettings.bind(this)}
              />
              <MovesSettings
                isOpen={this.state.moveSettingsOpen}
                toggle={this.toggleMovesSettings.bind(this)}
        ...
        <TableFooter>
          <TableRow>
            <TableCell colSpan="3">{this.props.tableFooter}</TableCell>
          </TableRow>
        </TableFooter>
      </Table>
    );
  ...
  getMultiItemRow(move, moveIndex) {
    return (
      <TableRow
        className={`${
          this.props.highlightMove === move.san ? "bgColor " : ""
        }moveRow`}
      ...
      >
        ...
        <TableCell
          size="small"
          id={`p${this.props.namespace}${moveIndex}`}
          className="smallCol"
          onClick={this.togglePerformancePopover(moveIndex)}
        >
          {simplifyCount(move.moveCount)}
          {this.getInfoIcon(moveIndex)}
        ...
        <TableCell>
          <Container>
            {this.props.settings.movesSettings.openingBookScoreIndicator ? (
              <Row className="scoresProgress">
                <Col className="navCol">{this.compareScores(move)}</Col>
              </Row>
            ) : null}
            <Row>
            ...
            {this.props.settings.movesSettings.openingBookWinsIndicator ? (
              <Row className="zeroHeight">
                <Col className="navCol">{this.compareProgress(move)}</Col>
              </Row>
            ) : null}
          </Container>
        </TableCell>
  ...
  getSingleItemRow(move, lastPlayedGame) {
    let sampleResultWhite = playerDetails(
      lastPlayedGame.white,
      lastPlayedGame.whiteElo
    );
    let sampleResultBlack = playerDetails(
      lastPlayedGame.black,
      lastPlayedGame.blackElo
    );
    ...
    return (
      ...
      >
        ...
        <TableCell colSpan="2">
          {sampleResultWhite} {sampleResult} {sampleResultBlack}{" "}
          {
            <FontAwesomeIcon
              className="pointerExternalLink"
              onClick={this.props.launchGame(move.details.lastPlayedGame)}
