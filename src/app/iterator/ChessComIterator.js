...
export default class ChessComIterator {
  constructor(
    ...
    let toYear = filterToDate
      ? filterToDate.getFullYear()
      : Number.MAX_SAFE_INTEGER;
    let toMonth = filterToDate ? filterToDate.getMonth() + 1 : 1;
    let minEpochTimeInSeconds =
      (filterFromDate ? filterFromDate.getTime() : 0) / 1000;
    let maxEpochTimeInSeconds =
      (filterToDate
        ? filterToDate.getTime() + Constants.MILLISECS_IN_DAY
    ...
    let parseGames = (archiveResponse) => {
      pendingRequests--;
      let continueProcessing = ready(
        archiveResponse.body.games
          .filter((game) => {
            if (
              game.end_time < minEpochTimeInSeconds ||
              game.end_time > maxEpochTimeInSeconds
            ) {
              return false;
            }
            if (
              game.rules !== Common.chessDotComRules(variant) ||
              game[playerColor].username.toLowerCase() !==
            ...
            let opponentFilter =
              advancedFilters[Constants.FILTER_NAME_OPPONENT];
            if (opponentFilter) {
              let opponent =
            ...
            let opponentElo =
              playerColor === Constants.PLAYER_COLOR_WHITE
                ...
                : game.white.rating;
            if (
              !isOpponentEloInSelectedRange(
                opponentElo,
                advancedFilters[Constants.FILTER_NAME_ELO_RANGE]
              )
            ) {
              return false;
            }
    ...
    let shouldFetchGamesFromArchive = (
      archiveMonthString,
      archiveYearString
    ) => {
    ...
    request(
      ...
      function (error, response, body) {
        if (error) {
          showError(
            "Failed to connect to chess.com. chess.com might be down at the moment",
            null,
            "Some addons like 'Piracy Badger' can also cause this."
          );
          ready([], false);
          ...
          if (response.body) {
            try {
              ...
              showError(
                "Could not find games for chess.com user " + playerName
              );
              ready([], false);
            }
          }
