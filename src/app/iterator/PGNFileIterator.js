...
export default class PGNFileIterator {
  ...
  setupReader(playerName, playerColor, advancedFilters, ready, showError) {
    ...
    let playerColorHeaderName =
      playerColor === Constants.PLAYER_COLOR_WHITE ? "White" : "Black";
    ...
    reader.onload = function (evt) {
      ...
      let parsedPGNs = pgnsArray.map((pgnString) => {
        try {
          ...
          if (
            playerName &&
            playerColorHeaderValue &&
            !playerColorHeaderValue.toLowerCase().includes(lowerCasePlayerName)
          ) {
